...
class FileMakerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public static function setUpBeforeClass()
    {
        ...
        if ($GLOBALS['OFFICIAL_API'] == 1) {
            self::$connection = new \FileMaker($GLOBALS['DB_FILE'], $GLOBALS['DB_HOST'], $GLOBALS['DB_USER'], $GLOBALS['DB_PASSWD']);
        } else {
            self::$connection = new FileMaker($GLOBALS['DB_FILE'], $GLOBALS['DB_HOST'], $GLOBALS['DB_USER'], $GLOBALS['DB_PASSWD'], ['errorHandling' => 'exception']);
        }
    }
    ...
    public function testNewPerformScriptCommand()
    {
        ...
        $result = $command->execute();
        if (!$GLOBALS['OFFICIAL_API'] && $GLOBALS['ENGINE'] == 'cwp') {
            $this->assertInstanceOf(Object\Result::class, $result);
        } elseif (!$GLOBALS['OFFICIAL_API']) {
            $this->assertArrayHasKey('scriptError', $result);
        } else {
            $this->assertInstanceOf(\FileMaker_Result::class, $result);
        }

        if ($GLOBALS['OFFICIAL_API'] == 'cwp') {
            $this->assertEquals(0, (int)$result->getFoundSetCount());
        }


        $command = $this->fm->newPerformScriptCommand('sample', 'create sample data', 50);
        if (!$GLOBALS['OFFICIAL_API'] && $GLOBALS['OFFICIAL_API'] == 'cwp' ) {
    ...
    {
        if ($GLOBALS['ENGINE'] = 'dataAPI') {
            $this->markTestSkipped('Find Any command not supported by dataAPI');
            return;
        }
    ...
    {
        $command = $this->fm->newFindCommand('sample');
        $command->addFindCriterion('id', '1');
        $command->setGlobal('global_field', 2);
        $result = $command->execute();
        if (!$GLOBALS['OFFICIAL_API']) {
            $this->assertInstanceOf(Object\Result::class, $result);
        } else {
        ...
        $record = $result->getFirstRecord();
        $this->assertEquals(2, $record->getField('related_sample_global::id'));

        $record2 = $this->fm->getRecordById('sample', $record->getRecordId());
        $this->assertEquals('', $record2->getField('related_sample_global::id'));
    }
    ...
    public function testNewCompoundFindCommand()
    {
        ...
        $command->addSortRule('id', 1, FileMaker::SORT_DESCEND);
        $result = $command->execute();
        if (!$GLOBALS['OFFICIAL_API']) {
            $this->assertInstanceOf(Object\Result::class, $result);
        } else {
        ...
        $this->assertEquals(20, $result->getFetchCount());

        $firstRecord = $result->getFirstRecord();
        $id = $firstRecord->getField('id');
        $this->assertEquals(50, $firstRecord->getField('id'));

        $command->setRange(0, 10);
        ...
        $this->assertEquals(10, $result2->getFetchCount());
    }
    ...
    {
        $query = $this->fm->newFindAllCommand('sample')
            ->setRange(0,1);
    ...
    {
        if ($GLOBALS['ENGINE'] == 'dataAPI') {
            $this->markTestSkipped('DataAPI already provide full conatiner URL');
            return;
        }
